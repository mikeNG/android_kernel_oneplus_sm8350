...
#define short_debug(fmt, ...)                                                  \
	do {                                                                   \
		if (short_d_on == true) {                                      \
			printk(KERN_CRIT pr_fmt("=====[OPLUS_CHG]" fmt),       \
			       ##__VA_ARGS__);                                 \
		}                                                              \
	} while (0)
...
static void oplus_short_c_batt_set_disable_rechg(struct oplus_chg_chip *chip,
						 bool disable)
{
...
static int oplus_short_c_batt_read_file(struct oplus_chg_chip *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(short_c_batt_items); i++) {
		...
		if (p != 0) {
			p = p + strlen(short_c_batt_items[i].name) +
			    strlen(":");
			strncpy(buf_tmp, p, oplus_digit_num_len(p));
			buf_tmp[oplus_digit_num_len(p)] = '\0';
			ret = kstrtouint(buf_tmp, 10,
					 &(short_c_batt_items[i].value));
			if (ret != 0) {
 ...
 */
static int oplus_short_c_batt_write_err_code(int err_code, int temp1, int temp2,
					     int vbatt1_mv, int vbatt2_mv,
					     int timer_counts)
{
	struct file *fp;
	mm_segment_t old_fs;
	...
	sprintf(buf, "err_code%d:%d,%d,%d,%d,%d,#", err_code, temp1, temp2,
		vbatt1_mv, vbatt2_mv, timer_counts);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pos = 0;
...
static int oplus_short_c_batt_write_exit_code(int exit_code, int temp1,
					      int temp2, int vbatt1, int vbatt2)
{
	struct file *fp;
	mm_segment_t old_fs;
	loff_t pos;
	...
	/*add ",#" and the end of the string for userspace*/
	sprintf(buf, "exit_code%d:%d,%d,%d,%d,#", exit_code, temp1, temp2,
		vbatt1, vbatt2);
...
void oplus_short_c_batt_update_change(struct oplus_chg_chip *chip,
				      int update_value)
{
...
static int oplus_short_c_batt_get_average_vbatt_mv(struct oplus_chg_chip *chip,
						   bool reset)
{
...
void oplus_chg_short_c_hw_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_shortc_hw_gpio_status() == 0) {
		...
		chg_debug(
			"[Shortc_HW] shorc_HW gpio low, count = %d, status = %d\n",
			short_c_hw_check_counts,
...
void oplus_chg_short_c_battery_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_is_power_off_chg(chip) == true) {
			...
			if (data_mounted_count < 30 &&
			    oplus_short_c_batt_data_is_mounted() == true) {
				oplus_short_c_batt_read_file(chip);
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD ||
	    chip->temperature > WARM_TEMP_THRESHOLD) {
		short_debug("temperature[%d]\n", chip->temperature);
		if (oplus_short_c_batt_get_cv_status(chip) == true) {
			oplus_short_c_batt_write_exit_code(
				EXIT_CODE__EX5_OUT_OF_TEMP_0_45, INVALID_CODE,
	...
	if (chip->batt_full == true && chip->chging_on == false &&
		...
		oplus_short_c_batt_write_chg_data(
			chip->icharging, chip->batt_volt, full_over_10ma_count,
			full_over_20ma_count, full_over_50ma_count);

		if (full_over_10ma_count >=
		    chip->short_c_batt.ex2_lower_ibatt_count) {
			oplus_short_c_batt_write_exit_code(
				EXIT_CODE__EX2_OVER_LOWER_IBATT, INVALID_CODE,
				INVALID_CODE, INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n",
				chip->short_c_batt.ex2_lower_ibatt_ma,
				chip->short_c_batt.ex2_lower_ibatt_count);
			short_debug(
				"don't exit_check, full_over_10ma_count[%d]\n",
				full_over_10ma_count);
			goto cv_reset;
		}
		if (full_over_20ma_count >=
		    chip->short_c_batt.ex2_low_ibatt_count) {
			oplus_short_c_batt_write_exit_code(
				EXIT_CODE__EX2_OVER_LOW_IBATT, INVALID_CODE,
				INVALID_CODE, INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n",
				chip->short_c_batt.ex2_low_ibatt_ma,
				chip->short_c_batt.ex2_low_ibatt_count);
			short_debug(
				"don't exit_check, full_over_20ma_count[%d]\n",
				full_over_20ma_count);
			goto cv_reset;
		}
		if (full_over_50ma_count >=
		    chip->short_c_batt.ex2_high_ibatt_count) {
			oplus_short_c_batt_write_exit_code(
				EXIT_CODE__EX2_OVER_HIGH_IBATT, INVALID_CODE,
				INVALID_CODE, INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n",
				chip->short_c_batt.ex2_high_ibatt_ma,
				chip->short_c_batt.ex2_high_ibatt_count);
			short_debug(
				"don't exit_check, full_over_50ma_count[%d]\n",
				full_over_50ma_count);
			goto cv_reset;
		}
		...
		if (full_timer5_count > chip->short_c_batt.full_timer5 &&
			...
			full_vbatt_2 = oplus_short_c_batt_get_average_vbatt_mv(
				chip, false);
			short_debug(
				"full_timer5_count overflow, full_vbatt_2[%d]\n",
				full_vbatt_2);
			if (full_vbatt_2 > 0 && chip->limits.iterm_ma != 0) {
				full_timer5_count =
					chip->short_c_batt.full_timer5 + 2;
				if (cv_avg_vbatt >
				...
				if (chip->limits.iterm_ma != 0) {
					rbatt = (cv_avg_vbatt - full_vbatt_2) *
						1000 / chip->limits.iterm_ma;
				} else {
					rbatt = -1;
				}
				if (chip->temperature <= COOL_TEMP_THRESHOLD) {
					if (rbatt >
					    chip->short_c_batt.cool_temp_rbatt) {
						exit_check_status = true;
					}
				} else if (chip->temperature <=
					   NORMAL_TEMP_THRESHOLD) {
					if (rbatt >
					    chip->short_c_batt
						    .little_cool_temp_rbatt) {
						exit_check_status = true;
					}
				} else {
					if (rbatt >
				...
				short_debug(
					"cv_avg_vbatt[%d], full_vbatt_2[%d], rbatt[%d]\n",
					cv_avg_vbatt, full_vbatt_2, rbatt);
				if (exit_check_status == true) {
					oplus_short_c_batt_write_exit_code(
						EXIT_CODE__OVER_RBATT,
						chip->temperature,
						chip->temperature, full_vbatt_2,
						rbatt);
					...
					oplus_short_c_batt_write_exit_code(
						EXIT_CODE__NOT_OVER_RBATT,
						chip->temperature,
						chip->temperature, full_vbatt_2,
						rbatt);
					short_debug(
						"exit_code: EXIT_CODE__NOT_OVER_RBATT\n");
				}
			}
			return;
		}
		...
		if (full_timer2_count > chip->short_c_batt.full_timer2 &&
			...
			full_vbatt_n = oplus_short_c_batt_get_average_vbatt_mv(
				chip, false);
			short_debug(
				"full_timer2_count overflow, full_vbatt_1[%d]\n",
				full_vbatt_n);
			if (full_vbatt_n > 0 && full_vbatt1 == 0) {
				full_timer2_count =
					chip->short_c_batt.full_timer2 + 2;
		...
		if (full_timer3_count <= chip->short_c_batt.full_timer3) {
			short_debug("full_timer3_count[%d]\n",
				    full_timer3_count);
			oplus_short_c_batt_get_average_vbatt_mv(chip, true);
			if (full_temp1 != INVALID_TEMP &&
			    abs(full_temp1 - chip->temperature) >
				    DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(
					EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER3,
					full_temp1, chip->temperature,
					INVALID_CODE, INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n",
					chip->temperature);
				short_debug(
					"don't exit_check, T1[%d], T2[%d]\n",
					full_temp1, chip->temperature);
				goto cv_reset;
			}
			return;
		...
		if (full_timer3_count > chip->short_c_batt.full_timer3 &&
			...
			full_vbatt_n = oplus_short_c_batt_get_average_vbatt_mv(
				chip, false);
			short_debug(
				"full_timer3_count overflow, full_vbatt_n[%d]\n",
				full_vbatt_n);
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer3_count ==
				    chip->short_c_batt.full_timer3 + 1) {
					if ((chip->short_c_batt
						     .batt_chging_cycles <
					     chip->short_c_batt
						     .batt_chging_cycle_threshold) &&
					    (full_vbatt1 - full_vbatt_n >
					     chip->short_c_batt
						     .full_delta_vbatt1_mv)) {
						short_debug(
							"set err_code[%d]\n",
							SHORT_C_BATT_STATUS__FULL_ERR_CODE2);
						chip->short_c_batt.err_code =
				...
				/* dynamic */
				if (full_timer3_count ==
				    chip->short_c_batt.full_timer3 + 2) {
					full_timer3_count =
						chip->short_c_batt.full_timer3 +
						3;
					if (chip->temperature <=
					...
					if (chip->short_c_batt.err_code !=
					    SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4) {
						short_debug(
							"exit_code, EXIT_CODE__TIMER3_OVERFLOW_INFO\n");
						oplus_short_c_batt_write_exit_code(
							EXIT_CODE__TIMER3_OVERFLOW_INFO,
							full_temp1,
							chip->temperature,
							full_vbatt1,
							full_vbatt_n);
		...
		if (full_timer4_count <= chip->short_c_batt.full_timer4) {
			...
			if (full_temp1 != INVALID_TEMP &&
			    abs(full_temp1 - chip->temperature) >
				    DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(
					EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER4,
					full_temp1, chip->temperature,
					INVALID_CODE, INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n",
					chip->temperature);
		...
		if (full_timer4_count > chip->short_c_batt.full_timer4 &&
			...
			full_vbatt_n = oplus_short_c_batt_get_average_vbatt_mv(
				chip, false);
			short_debug(
				"full_timer4_count overflow, full_vbatt_n[%d]\n",
				full_vbatt_n);
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer4_count ==
				    chip->short_c_batt.full_timer4 + 1) {
					if ((chip->short_c_batt
						     .batt_chging_cycles <
					     chip->short_c_batt
				...
				/* dynamic */
				if (full_timer4_count ==
				    chip->short_c_batt.full_timer4 + 2) {
					full_timer4_count =
						chip->short_c_batt.full_timer4 +
						3;
					if (chip->temperature <=
					...
					if (chip->short_c_batt.err_code !=
					    SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5) {
						short_debug(
							"exit_check, EXIT_CODE__TIMER4_OVERFLOW\n");
						oplus_short_c_batt_write_exit_code(
							EXIT_CODE__TIMER4_OVERFLOW,
							full_temp1,
							chip->temperature,
							full_vbatt1,
							full_vbatt_n);
	...
	if (chip->batt_volt > chip->short_c_batt.short_c_bat_cv_mv &&
		...
		if (cv_pre_check_count == CV_PRE_CHECK_COUNT) {
			if (chip->short_c_batt.cv_satus == false) {
				oplus_short_c_batt_write_exit_code(0, 0, 0, 0,
								   0);
				chip->short_c_batt.cv_satus = true;
	...
	if (chip->short_c_batt.cv_satus == true) {
		...
		if (chip->batt_volt <= chip->short_c_batt.short_c_bat_cv_mv ||
		    (chip->icharging < 0 &&
		     (-1 * chip->icharging) >= CV_IBATT_MA_THRESHOLD) ||
		    abs(pre_icharging - chip->icharging) >=
			    CV_DELTA_IBATT_MA_THRESHOLD) {
			...
			if ((-1 * chip->icharging) <
			    chip->limits.iterm_ma + 50) {
				cv_iterm_count++;
				if (cv_vbatt_count < CV_VBATT_COUNT) {
					short_debug(
						"cv_iterm_count[%d], cv_avg_vbatt[%d]\n",
			...
			if (cv_iterm_count == CV_ITERM_COUNT &&
			    cv_vbatt_count < CV_VBATT_COUNT) {
				/* 2017-12-18 use the max value instead of avg value */
				cv_avg_vbatt =
					max(cv_avg_vbatt, chip->batt_volt);
				cv_vbatt_count++;
				short_debug(
					"cv_vbatt_count[%d], max cv_avg_vbatt[%d]\n",
		...
		if (cv_timer1_count == chip->short_c_batt.cv_timer1) {
			...
			chip->short_c_batt.err_code =
				SHORT_C_BATT_STATUS__CV_ERR_CODE1;
			oplus_short_c_batt_write_err_code(
				SHORT_C_BATT_STATUS__CV_ERR_CODE1, cv_temp,
				chip->temperature, INVALID_CODE, INVALID_CODE,
				INVALID_CODE);
